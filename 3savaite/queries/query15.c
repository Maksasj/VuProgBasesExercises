...
Student bestStudent(Student students[], int count) {
	Student tmpStudent = students[0];

	for(int i = 0; i < count; i++) {
		if(tmpStudent.average < students[i].average) {
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			Student b = bestStudent(students, size);

			if(b.average - 1.0 <= students[i].average){ // *** first filter, conditions on the student
