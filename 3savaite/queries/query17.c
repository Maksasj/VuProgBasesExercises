...
int differentMarksForSameCourse(Student s) {
	for(int i = 0; i < s.load; i++) {
		int tmpMark1 = s.grades[i];
		int tmpMarkIndex2 = ifArrayHaveSuch(s.courses, s.load, s.courses[i]);
		if(tmpMarkIndex2 != -1) {
			if(tmpMark1 != s.grades[tmpMarkIndex2]) {
				return 1;
			}
		}
	}
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			/*
			I have looked into generator.c and looks like this is impossible for Student to get one languages twice.
			*/

			if(differentMarksForSameCourse(students[i])){ // *** first filter, conditions on the student
