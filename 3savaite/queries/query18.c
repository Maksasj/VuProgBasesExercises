...
int uniqueName(Student students[], int count, Student student) {
	int tmp = 1;

	for(int i = 0; i < count; i++) {
		int equal = (strcmp(students[i].name, student.name) == 0);
...
int uniqueSurname(Student students[], int count, Student student) {
	int tmp = 1;

	for(int i = 0; i < count; i++) {
		int equal = (strcmp(students[i].surname, student.surname) == 0);
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(uniqueSurname(students, size, s) || uniqueName(students, size, s)){ // *** first filter, conditions on the student
