...
int nameLongerThenSurname(char name[], char surname[]) {
	return stringLength(name, 30) > stringLength(surname, 30);
}
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(nameLongerThenSurname(students[i].name, students[i].surname)){ // *** first filter, conditions on the student
