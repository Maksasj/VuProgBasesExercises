...
int TopBound(int grades[], int load, int mark) {
...
int BottomBound(int grades[], int load, int mark) {
	for(int i = 0; i < load; i++) {
		if (grades[i] < mark) {
			return 0;
		}
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(	TopBound(students[i].grades, students[i].load, 9) &&
				BottomBound(students[i].grades, students[i].load, 6)){ // *** first filter, conditions on the student
