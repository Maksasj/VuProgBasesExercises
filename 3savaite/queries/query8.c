...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(	ifArrayHaveSuch(students[i].courses, students[i].load, "Calculus") &&
				ifArrayHaveSuch(students[i].courses, students[i].load, "Algebra")) { // *** first filter, conditions on the student
