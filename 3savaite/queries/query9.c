...
int marksFromSubj(char courses[10][30], int marks[10], int load, char course[]) {
	int p = 1000;
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(	ifArrayHaveSuch(students[i].courses, students[i].load, "Philosophy") &&
			
				4 < marksFromSubj(students[i].courses, students[i].grades, students[i].load, "Philosophy") && 
				marksFromSubj(students[i].courses, students[i].grades, students[i].load, "Philosophy") != 1000
